...
export const useAppointmentForm = (onClose: () => void, appointmentId?: string) => {
  ...
  const handleConsultationTypeChange = (consultationTypeIds: string[], consultationTypes: any[]) => {
    console.log('🔄 Consultation types changed:', consultationTypeIds);
    
    // Calculer la durée totale de toutes les consultations sélectionnées
    const totalDuration = consultationTypes
      .filter(type => consultationTypeIds.includes(type.id))
      .reduce((sum, type) => sum + type.duration_minutes, 0);
    ...
    updateField('duration', totalDuration || 15);
  ...
  const initializeFormData = (defaultData: any) => {
    ...
    if (defaultData) {
      ...
      // Gérer à la fois l'ancien format (single ID) et le nouveau format (array)
      ...
      } else if (defaultData.consultation_type_ids && Array.isArray(defaultData.consultation_type_ids)) {
        cleanData.consultationTypeIds = defaultData.consultation_type_ids;
      } else if (defaultData.consultationTypeId) {
        // Rétrocompatibilité: convertir l'ancien format en array
        cleanData.consultationTypeIds = [defaultData.consultationTypeId];
      } else if (defaultData.consultation_type_id) {
        cleanData.consultationTypeIds = [defaultData.consultation_type_id];
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const appointmentData = {
        ...
        consultation_type_ids: formData.consultationTypeIds.length > 0 ? formData.consultationTypeIds : null,

...
interface CentralizedAbsenceManagerProps {
...
export const CentralizedAbsenceManager: React.FC<CentralizedAbsenceManagerProps> = ({
  veterinarians,
  absences
}) => {
  const { addAbsence, deleteAbsence } = useVeterinarianAbsences();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedVeterinarianFilter, setSelectedVeterinarianFilter] = useState<string>("");
  ...
  const filteredAbsences = selectedVeterinarianFilter 
    ? absences.filter(absence => absence.veterinarian_id === selectedVeterinarianFilter)
    : absences;

  const handleAddAbsence = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!newAbsence.veterinarian_id || !newAbsence.start_date || !newAbsence.end_date || !newAbsence.absence_type) {
      return;
    }

    const success = await addAbsence({
      veterinarian_id: newAbsence.veterinarian_id,
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-col sm:flex-row gap-4">
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Définissez une période d'absence pour un vétérinaire
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleAddAbsence}>
                <div className="grid gap-4 py-4">
                  <div>
                    <Label htmlFor="veterinarian-select">Vétérinaire</Label>
                    <Select
                      value={newAbsence.veterinarian_id}
                      onValueChange={(value) => setNewAbsence(prev => ({ ...prev, veterinarian_id: value }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Sélectionner un vétérinaire" />
                      </SelectTrigger>
                      <SelectContent className="bg-white border-vet-blue/30 shadow-lg z-50">
          ...
          <div className="flex-1">
            <Label htmlFor="vet-filter">Filtrer par vétérinaire</Label>
            <Select
              value={selectedVeterinarianFilter}
              onValueChange={setSelectedVeterinarianFilter}
            >
              <SelectTrigger>
            ...
            <div className="text-center py-8 text-vet-brown bg-vet-beige/10 rounded-lg border border-vet-blue/20">
              ...
              <p className="text-sm">
                {selectedVeterinarianFilter 
                  ? "Aucune absence pour ce vétérinaire"
                  : "Ajoutez des absences pour bloquer des créneaux"
                }
              </p>
            </div>
          ) : (

...
const ConditionalQuestionsContent = ({
  ...
  const hasSecondAnimalSymptoms = hasTwoAnimals && (bookingData.secondAnimalSelectedSymptoms?.length > 0 || bookingData.secondAnimalCustomSymptom?.trim() !== '') && bookingData.secondAnimalConsultationReason === 'symptomes-anomalie';
  return <>
      {/* Formulaire */}
      <Card className="bg-white/95 backdrop-blur-sm border-vet-blue/20 shadow-lg relative">
        <CardContent className="p-3 sm:p-6">
          <div className="space-y-4 sm:space-y-6">
            ...
            {hasFirstAnimalSymptoms && <div className="space-y-4">
                ...
                <ConditionalQuestionsForm selectedSymptoms={bookingData.selectedSymptoms || []} customSymptom={bookingData.customSymptom || ''} onAnswersChange={onAnswersChange} animalPrefix="animal1_" />
              </div>}

            {/* Questions pour l'animal 2 - Seulement si 2 animaux sélectionnés */}
            {hasSecondAnimalSymptoms && <div className="space-y-4">
                ...
                <ConditionalQuestionsForm selectedSymptoms={bookingData.secondAnimalSelectedSymptoms || []} customSymptom={bookingData.secondAnimalCustomSymptom || ''} onAnswersChange={onAnswersChange} animalPrefix="animal2_" />
              </div>}

            {!hasAnyConditions && <div className="text-center text-vet-brown/60 py-8">
          ...
          {!isMobile && <div className="flex justify-end mt-6 pt-4 border-t border-gray-100">
              <Button onClick={onNext} disabled={!canProceed} className="bg-vet-sage hover:bg-vet-sage/90 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-2 text-sm font-medium rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                Continuer
      ...
      {isMobile && <div className="fixed bottom-6 right-6 z-50">
          <Button onClick={onNext} disabled={!canProceed} className="bg-vet-sage hover:bg-vet-sage/90 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2 text-sm font-medium rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
            Continuer

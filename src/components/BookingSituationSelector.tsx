...
const BookingSituationSelector: React.FC<BookingSituationSelectorProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid gap-3 transition-all duration-300 ${selectedSituation ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-3'}`}>
        {/* Option 1 animal */}
        {(!selectedSituation || selectedSituation === '1-animal') && (
          <div 
            className={`relative flex flex-row items-center rounded-lg border-2 transition-all duration-300 cursor-pointer hover:shadow-md ${
              selectedSituation === '1-animal' 
                ? 'border-primary bg-primary/5 shadow-md p-3' 
                : 'border-gray-200 bg-white hover:border-primary/50 p-3'
            }`}
            onClick={() => handleCardClick('1-animal')}
          >
            <Dog className={`transition-all duration-300 ${
              selectedSituation === '1-animal' ? 'w-6 h-6 mr-3 text-primary' : 'w-7 h-7 mr-3 text-muted-foreground'
            }`} />
            <div className="flex-1 text-left">
              <Label className="text-vet-navy font-semibold cursor-pointer block">
        ...
        {(!selectedSituation || selectedSituation === '2-animaux') && (
          ...
          >
            <div className={`relative transition-all duration-300 ${
              selectedSituation === '2-animaux' ? 'w-6 h-6 mr-3' : 'w-7 h-7 mr-3'
            }`}>
              <PawPrint className={`absolute top-0 left-0 transition-all duration-300 ${
                selectedSituation === '2-animaux' ? 'w-4 h-4 text-primary' : 'w-4 h-4 text-muted-foreground'
              }`} />
        ...
        {(!selectedSituation || selectedSituation === 'une-portee') && (
          ...
          >
            <Heart className={`transition-all duration-300 fill-current ${
              selectedSituation === 'une-portee' ? 'w-6 h-6 mr-3 text-primary' : 'w-7 h-7 mr-3 text-muted-foreground'
            }`} />
            <div className="flex-1 text-left">
              <Label className="text-vet-navy font-semibold cursor-pointer block">

...
-- Create a table for veterinary practice requests
...
-- Add Row Level Security (RLS) to allow all operations (since this is for admin management)
ALTER TABLE public.veterinary_practice_requests ENABLE ROW LEVEL SECURITY;

-- Create policy that allows all operations (for now, we'll manage access through the application)
